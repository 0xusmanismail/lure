...
ARM64 support:
  - Detected via pyelftools (EM_AARCH64 = 183) using get_binary_arch()
  - Requires qemu-aarch64 on PATH (qemu-user package on Arch Linux)
  - strace wraps the entire QEMU invocation: strace ... qemu-aarch64 ./binary
  - lure-wrapper receives qemu-aarch64 as argv[1] and execs into it
  - Report shows "Architecture: ARM64 (QEMU emulated)" in Execution Summary
...
from lure.inspector import is_elf_file, NOT_ELF_ERROR, get_binary_arch
...
def main():
    # argv layout (8 minimum required, '--' at position 7):
    #   [0] script
    #   [1] root_dir
    #   [2] trace_path
    #   [3] marker_path
    #   [4] no_seccomp_marker_path
    #   [5] binary_path
    #   [6] qemu_binary  (empty string '' for native x86-64 execution)
    #   [7] '--'
    #   [8..] extra_argv
    if len(sys.argv) < 8 or sys.argv[7] != '--':
        fail('internal error: bad argv from lure')

    root_dir, trace_path, marker_path, no_seccomp_marker_path, binary_path, qemu_binary = sys.argv[1:7]
    extra_argv = sys.argv[8:]
    ...
    # ── strace command assembly ───────────────────────────────────────────────
    #
    # Four branches depending on whether the guest_wrapper (seccomp) is
    # available and whether a QEMU binary is needed (ARM64 guests):
    #
    #   wrapper + native:  strace -> wrapper -> /{guest_name}
    #   wrapper + qemu:    strace -> wrapper -> qemu-aarch64 -> /{guest_name}
    #   no wrapper + native:  strace -> /{guest_name}
    #   no wrapper + qemu:    strace -> qemu-aarch64 -> /{guest_name}
    #
    # For the wrapper+qemu case, the wrapper receives qemu_binary as
    # argv[1] (the path to execv into) and /{guest_name} as argv[3]
    # (passed to qemu as the ARM64 binary argument). The wrapper does:
    #   execv(qemu_binary, [qemu_binary, "/{guest_name}", ...])
    # which is the correct qemu-user invocation.
    ...
    if not wrapper_ready:
        ...
        if qemu_binary:
            # wrapper argv[1] = qemu_binary (what to execv into)
            # wrapper argv[2] = no_seccomp_fd (the marker)
            # wrapper argv[3] = /{guest_name} (passed to qemu as the binary)
            strace_cmd = [
                'strace', '-f', '-tt', '-T', '-e', 'trace=all',
                '-o', f'/proc/self/fd/{trace_fd}',
                '--', wrapper_bin_path, qemu_binary, str(no_seccomp_fd),
                f'/{guest_name}',
            ] + extra_argv
        else:
            strace_cmd = [
        ...
        with open(trace_path, 'r', errors='replace') as fh:
            for raw in fh:
                ...
                if evt['type'] == 'file':
                    ...
                    # lure-wrapper is our own internal seccomp-installer
                    # (see _ROOT_INIT_SRC / guest_wrapper) — strace sees
                    # it exec because it's the actual traced process,
                    # but it's an implementation detail, not something
                    # the guest itself spawned. Hide it from the report.
                    if evt['path'] == '/tmp/lure-wrapper' or evt['path'].startswith('/tmp/lure-wrapper'):
                        pass
                    else:
                        cmd = evt['cmd']
...
def _render_report(
    binary_path, exit_code, elapsed, timed_out, events,
    program_output, isolation, resource_limits, arch_display=None,
):
    """Print all report sections after the binary exits.

    arch_display: when set (e.g. 'ARM64 (QEMU emulated)'), adds an
    Architecture row to the Execution Summary panel. None for native
    x86-64 runs — keeps the report identical to pre-v0.7.0 output.
    """
    ...
    if arch_display:
        t.add_row('Architecture', arch_display)
    t.add_row('Exit Code',  exit_display)
    t.add_row('Runtime',    f'{elapsed:.3f}s')
    t.add_row('Isolation',  isolation)
    if resource_limits.get('cgroups_active'):
        if resource_limits.get('memory_max_applied'):
            t.add_row('Memory limit',  f'{_CGROUP_MEMORY_MAX_MB} MB')
        else:
            t.add_row('Memory limit',  Text('not applied', style='dim italic'))
        if resource_limits.get('pids_max_applied'):
            t.add_row('Process limit', str(_CGROUP_PIDS_MAX_INT))
        else:
            t.add_row('Process limit', Text('not applied', style='dim italic'))
    else:
        t.add_row('Resource limits', Text('not available', style='dim italic'))
...
def _save_report(
    binary_path, exit_code, elapsed, timed_out, events,
    label, triggers, isolation, resource_limits, arch_display=None,
):
    """
    ...
    arch_display: when set, included in the JSON report as
    'architecture' key (e.g. 'ARM64 (QEMU emulated)').
    """
    try:
    ...
    if resource_limits.get('cgroups_active'):
        json_resource_limits = {
            'memory_max':     f'{_CGROUP_MEMORY_MAX_MB}m',
            'pids_max':       _CGROUP_PIDS_MAX_INT,
            'cgroups_active': True,
        }
    else:
        json_resource_limits = {'cgroups_active': False}
    ...
    report_data = {
        ...
        'isolation':         isolation,
        'resource_limits':   json_resource_limits,
        'verdict':           label,
    ...
    # Optional: architecture field (ARM64 runs only)
    if arch_display:
        report_data['architecture'] = arch_display
...
#
# Best-effort memory/PID/swap limits for the guest, applied via a
# per-session cgroup. This is deliberately layered OUTSIDE the
# mount/PID-namespace + seccomp machinery above: cgroup membership is
# inherited automatically by every descendant process across
# fork/exec, so wrapping just the outermost command (the full
# `unshare ...` invocation, or the fallback `strace ...` invocation)
# is sufficient to cover the entire chain — unshare, the init script,
# strace, guest_wrapper, and the guest itself — with no changes needed
# to any of those inner layers.
#
# On Arch Linux (and most non-systemd-managed setups), the cgroup2
# tree is not writable by unprivileged users without explicit
# delegation — see the README section this task adds, which has the
# user delegate a SUBDIRECTORY (/sys/fs/cgroup/lure), not the mount
# root itself (chown-ing the root of the whole cgroup2 hierarchy would
# be far too broad a grant). Every check and cgroup creation below
# therefore targets that subdirectory, not /sys/fs/cgroup directly —
# only the filesystem-TYPE check needs the mount root, since a
# subdirectory of a cgroup2 mount is always cgroup2 itself and may not
# exist yet on a system that hasn't delegated anything. Every failure
# mode here is non-fatal: 'lure run' always proceeds, with or without
# limits, and reports honestly which limits (if any) actually applied.

_CGROUP_MEMORY_MAX      = '536870912'  # 512 MiB, in bytes
_CGROUP_MEMORY_MAX_MB   = 512
_CGROUP_PIDS_MAX        = '64'
_CGROUP_PIDS_MAX_INT    = 64
_CGROUP_MOUNT_ROOT      = '/sys/fs/cgroup'
_CGROUP_BASE            = '/sys/fs/cgroup/lure'


def _cgroups_v2_available():
    """
    True only if the cgroup2 filesystem is mounted at all AND a
    session cgroup can actually be created for us to use. Order
    matters: on hybrid v1/v2 setups (common on older distros and some
    containers) /sys/fs/cgroup itself is a plain tmpfs that happily
    accepts mkdir/rmdir, which would give a false positive if the
    write-test ran without first confirming the filesystem type.

    The write-test targets _CGROUP_BASE (the delegated subdirectory
    the README has the user set up), not _CGROUP_MOUNT_ROOT — a real
    delegation only chowns the subdirectory, and testing the mount
    root itself would report "unavailable" even on a correctly
    delegated system.
    """
    try:
        result = subprocess.run(
            ['stat', '-f', '-c', '%T', _CGROUP_MOUNT_ROOT],
    ...
    test_path = os.path.join(_CGROUP_BASE, f'lure-test-{uuid.uuid4().hex[:8]}')
    try:
        os.mkdir(test_path)
        os.rmdir(test_path)
    except OSError:
...
def _setup_cgroup(session_uuid):
    """
    Create /sys/fs/cgroup/lure-<session_uuid>/ and apply memory/pids/
    swap limits. Each limit is written independently and best-effort —
    a delegated cgroup that only has SOME controllers enabled (common)
    still gets whatever limits it can support, rather than an
    all-or-nothing failure.

    Returns (cgroup_path_or_None, limits_dict). limits_dict always has
    'cgroups_active' (bool — true only if the cgroup itself was
    created, regardless of which individual limits succeeded) and, if
    active, 'memory_max_applied' / 'pids_max_applied' (bool each).
    """
    cgroup_path = os.path.join(_CGROUP_BASE, f'lure-{session_uuid}')
    try:
        os.mkdir(cgroup_path)
    except OSError:
        return None, {'cgroups_active': False}

    limits = {'cgroups_active': True, 'memory_max_applied': False, 'pids_max_applied': False}

    try:
        with open(os.path.join(cgroup_path, 'memory.max'), 'w') as f:
            f.write(_CGROUP_MEMORY_MAX)
        limits['memory_max_applied'] = True
    except OSError:
        pass
    ...
    try:
        with open(os.path.join(cgroup_path, 'pids.max'), 'w') as f:
            f.write(_CGROUP_PIDS_MAX)
        limits['pids_max_applied'] = True
    except OSError:
        pass

    return cgroup_path, limits


def _wrap_cmd_with_cgroup(cmd, cgroup_path):
    """
    Wraps `cmd` so the process that ultimately execs it has already
    added its own PID to cgroup.procs first — closing the race window
    where the guest chain could run briefly unconfined before being
    placed in the cgroup. Membership is inherited by every descendant
    across fork/exec, so this needs to wrap only the OUTERMOST command
    (e.g. the full `unshare ...` invocation), never the inner layers.

    The cgroup.procs path is our own generated path (never user
    input), and the real command is passed through via "$@" — actual
    argv elements, never string-interpolated — so this is not subject
    to shell injection from anything in `cmd` itself.
    """
    procs_file = os.path.join(cgroup_path, 'cgroup.procs')
    return [
        '/bin/sh', '-c',
        f'echo $$ > {shlex.quote(procs_file)} 2>/dev/null; exec "$@"',
        '--',
    ] + cmd


def _cleanup_cgroup(cgroup_path):
    """
    Kill any processes still lingering in the cgroup (a timed-out or
    killed guest may leave orphaned children behind that `proc.kill()`
    alone wouldn't reach), then remove the cgroup directory. Every
    step is best-effort: cleanup errors are logged at most, and never
    propagate — a run that succeeded in every other respect must not
    be reported as failed just because cgroup teardown had trouble.
    """
    if not cgroup_path:
        return
...
def _execute(
    ...
    qemu_binary=None, arch_display=None,
):
    ...
    qemu_binary:  path to qemu-aarch64 for ARM64 guests, or None for
                  native x86-64 execution.
    arch_display: human-readable architecture string for the report header,
                  e.g. 'ARM64 (QEMU emulated)', or None for native runs.
    """
    ...
    # Pass qemu_binary (or empty string for native) as the new argv[6]
    # slot in the init script. The '--' sentinel moves to argv[7].
    full_cmd = (
        ['unshare'] + unshare_ns_flags + ['--',
         sys.executable, init_script_path,
         root_dir, tmp_trace, marker_path, no_seccomp_marker_path, binary_path,
         qemu_binary or '',   # argv[6]: qemu path or '' for native
         '--']                # argv[7]: sentinel (was argv[6] before v0.7.0)
        + binary_argv
    )
    ...
    # Fallback: user+net namespace only, no mount/pid namespace, no chroot.
    # Used if mount-namespace setup fails, or if `unshare` isn't installed.
    # guest_cmd prepends qemu_binary for ARM64 guests in all fallback paths.
    strace_base_fallback = ['strace'] + strace_flags + ['-o', tmp_trace, '--']
    guest_cmd = ([qemu_binary, binary_path] if qemu_binary else [binary_path]) + binary_argv

    if shutil.which('unshare'):
        if allow_net:
            fallback_cmd   = ['unshare', '--user', '--'] + strace_base_fallback + guest_cmd
            fallback_label = 'user only (mount namespace unavailable, network allowed)'
        else:
            fallback_cmd   = ['unshare', '--user', '--net', '--'] + strace_base_fallback + guest_cmd
            fallback_label = 'user + net (mount namespace unavailable)'
    else:
        fallback_cmd   = strace_base_fallback + guest_cmd
        fallback_label = 'none (--allow-net)' if allow_net else 'none (unshare not found)'

    use_mount_ns = shutil.which('unshare') is not None

    # ── cgroups v2 resource limits ───────────────────────────────────────────
    # Applied by wrapping the OUTERMOST command (whichever one actually
    # launches — full_cmd or fallback_cmd) with a shell that adds its
    # own PID to the cgroup before exec-ing into the real chain. Every
    # descendant across unshare/fork/exec inherits membership
    # automatically. Entirely independent of, and unaffected by,
    # whether mount-namespace setup itself succeeds or falls back.
    session_uuid = uuid.uuid4().hex[:12]
    cgroup_path  = None
    cgroup_limits = {'cgroups_active': False}
    if _cgroups_v2_available():
        cgroup_path, cgroup_limits = _setup_cgroup(session_uuid)
        if cgroup_path:
            full_cmd     = _wrap_cmd_with_cgroup(full_cmd, cgroup_path)
            fallback_cmd = _wrap_cmd_with_cgroup(fallback_cmd, cgroup_path)
    else:
        console.print(
            '[dim]Note: cgroups unavailable — resource limits not enforced. '
            'To enable: see README.[/dim]'
        )
    ...
    label, triggers = _render_report(
        binary_path, exit_code, elapsed, timed_out, events,
        tmp_output, iso_label, cgroup_limits, arch_display,
    )
    ...
    if save:
        result = _save_report(
            binary_path, exit_code, elapsed, timed_out, events,
            label, triggers, iso_label, cgroup_limits, arch_display,
        )
        if result:
...
def run_binary(binary, timeout, binary_args, allow_net, trace_out, save=False):
    ...
    # ── Architecture detection for QEMU emulation ─────────────────────────────
    #
    # Detect the binary's architecture using pyelftools (same mapping as
    # lure inspect). For ARM64 guests, qemu-aarch64 is required on PATH;
    # strace wraps the entire QEMU invocation using -f to follow forks.
    # For native x86-64, qemu_binary stays None and everything proceeds
    # exactly as in v0.6.0 — zero change to the native code path.
    arch         = get_binary_arch(binary_path)
    qemu_binary  = None
    arch_display = None

    if arch == 'ARM64':
        qemu_binary = shutil.which('qemu-aarch64')
        if qemu_binary is None:
            console.print(
                '[red]Error:[/red] ARM64 binary detected but qemu-aarch64 not found.'
            )
            console.print(
                '[dim]Install with:[/dim] sudo pacman -S qemu-user'
            )
            sys.exit(1)
        arch_display = 'ARM64 (QEMU emulated)'
